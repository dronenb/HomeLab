...
function install_prereqs {
    ...
    if [[ "${KIND_INSTALLED}" -ne 1  ]]; then
        pkgs_to_install+=("kind")
    fi
...
function bootstrap_kind {
    while true; do
        echo -ne "${YELLOW}This process will delete and recreate any existing kind cluster.\nWould you like to continue? [y/n]: ${NC}"
        read -r yn
        case ${yn} in
            [Yy]* ) break;;
            [Nn]* ) exit 0;;
    ...
    if kind delete cluster > /dev/null 2>&1; then
        echo -e "${YELLOW}Deleted existing kind cluster${NC}"
    fi
